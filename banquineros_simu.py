...
csvFileName = f'csvs4/output.csv'
...
def runSimulationAndSave(speedFactor: float, sigma: float, lcStrategic: float, lcCooperative: float, lcSpeedGain: float, lcOvertakeRight: float):
    ...
    for vehsPerHour in range(1000, 15001, 1000):
        ...
        newVehicleType.set('id', 'banquinero')
        newVehicleType.set('vClass', 'custom1')
        ...
        newVehicleType.set('desiredMaxSpeed', '41.67')

        newFlow = ET.Element('flow')
        newFlow.set('id', 'f_5')
        newFlow.set('type', 'banquinero')
        newFlow.set('departLane', '0')
